#ifndef OPEN_SPIEL_GAMES_ENGLISHMAN_H_
#define OPEN_SPIEL_GAMES_ENGLISHMAN_H_
...
class EnglishmanState : public State {
 public:
  EnglishmanState(std::shared_ptr<const Game> game);

  EnglishmanState(const EnglishmanState&) = default;
  EnglishmanState& operator=(const EnglishmanState&) = default;
...
// Game object.
class EnglishmanGame : public Game {
 public:
  explicit EnglishmanGame(const GameParameters& params);
  int NumDistinctActions() const override { return kNumCells; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new EnglishmanState(shared_from_this()));
  }
